...
func SaveConnection(db *sql.DB, chatID int64, provider, repoURL, gitToken, defaultBranch string) error {
	_, err := db.Exec(`INSERT INTO connections (chat_id, provider, repo_url, git_token, default_branch) VALUES ($1, $2, $3, $4, $5)`,
		chatID, provider, repoURL, encryptToken(gitToken), defaultBranch)
	return err
}

func ListConnections(db *sql.DB, chatID int64) ([]*Connection, error) {
	rows, err := db.Query(`SELECT id, chat_id, provider, repo_url, git_token, default_branch, agent, agent_model, last_polled_at FROM connections WHERE chat_id = $1`, chatID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func DeleteAllConnections(db *sql.DB, chatID int64) error {
	_, err := db.Exec(`DELETE FROM connections WHERE chat_id = $1`, chatID)
	return err
}

func GetConnectionByID(db *sql.DB, id int64) (*Connection, error) {
	row := db.QueryRow(`SELECT id, chat_id, provider, repo_url, git_token, default_branch, agent, agent_model, last_polled_at FROM connections WHERE id = $1`, id)
	c := &Connection{}
	var lastPolled sql.NullTime
...
func UpdateConnectionDefaultBranch(db *sql.DB, chatID int64, repoURL, defaultBranch string) error {
	_, err := db.Exec(`UPDATE connections SET default_branch = $1 WHERE chat_id = $2 AND repo_url = $3`,
		defaultBranch, chatID, repoURL)
	return err
}

func FindConnectionByRepo(db *sql.DB, chatID int64, repoURL string) (*Connection, error) {
	row := db.QueryRow(`SELECT id, chat_id, provider, repo_url, git_token, default_branch, agent, agent_model, last_polled_at FROM connections WHERE chat_id = $1 AND repo_url = $2`,
		chatID, repoURL)
	c := &Connection{}
	var lastPolled sql.NullTime
	err := row.Scan(&c.ID, &c.ChatID, &c.Provider, &c.RepoURL, &c.GitToken, &c.DefaultBranch, &c.Agent, &c.AgentModel, &lastPolled)
...
func SetConnectionAgent(db *sql.DB, id int64, agent, agentModel string) error {
	_, err := db.Exec(`UPDATE connections SET agent = $1, agent_model = $2 WHERE id = $3`, agent, agentModel, id)
	return err
}

func UpdateConnectionToken(db *sql.DB, chatID int64, repoURL, newToken string) error {
	_, err := db.Exec(`UPDATE connections SET git_token = $1 WHERE chat_id = $2 AND repo_url = $3`,
...
func UpdateConnectionLastPolled(db *sql.DB, id int64, t time.Time) error {
	_, err := db.Exec(`UPDATE connections SET last_polled_at = $1 WHERE id = $2`, t, id)
	return err
}
